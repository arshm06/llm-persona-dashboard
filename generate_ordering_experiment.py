...
async def main():
    ...
    # Prepare persona templates with formatting mapped down the line
    for index, row in sampled_df.iterrows():
        ...
        for iteration in range(ITERATIONS_PER_ROW):
            for ordering_name, template in PROMPT_TEMPLATES.items():
                ...
                sim_id = f"P{index}_R{iteration}_{ordering_name.replace(' ', '').replace('->', '_')}"
                
                for q_id, q_text in IPIP_ITEMS.items():
                    all_tasks.append((sim_id, row, ordering_name, ask_single_question(persona_desc, q_id, q_text, semaphore)))

    print(f"Launching {len(all_tasks)} isolated requests across combinations...")
    
    results = []
    raw_results = await tqdm.gather(*(t[3] for t in all_tasks))
    
    final_rows = []
    for i in range(0, len(all_tasks), 50): # 50 questions per template per user
        chunk = raw_results[i : i + 50]
        sim_id, original_row, ordering_name, _ = all_tasks[i]
